...
def start_gui(args,configname=None):
...
class gui():
    ...
    def __init__(self,args,configname=None):
        ...
        self.args = None
        self.debug = 0
        self.update_intervall = self.configs.values.get('gui','update_intervall') # milliseconds
        self.check_buttons_intervall = self.configs.values.get('gui','check_buttons_intervall') # milliseconds
        ...
        self.cameras = []
        for i in range(self.configs.number_of_cameras):
            self.cameras += [camera.camera(config=self.configs,confsect="camera%d" % (i+1),pw=self.camera_tabs[i],screenx=self.screenx,screeny=self.screeny,extern_img=self.cameras_imgs[i],extern_x=self.camera_picture_x,notebook=self.tabs,notebookindex=self.camera_notebook_index[i],notebookextern=0)]
        # create block for acceleration sensor
        self.acceleration_sensor = []
        for i in range(self.configs.number_of_acceleration_sensor):
            self.acceleration_sensor += [acceleration_sensor.acceleration_sensor(config=self.configs,confsect="acceleration_sensor%d" % (i+1),pw=self.acceleration_sensor_tabs[i])]
            self.acceleration_sensor[i].extern_update_start(extern_stringvar=self.info_area_acceleration_sensor_vals[i],notebook=self.tabs,notebookindex=self.acceleration_sensor_notebook_index[i],notebookextern=0)
        # create other blocks
        ...
        if self.translation_stage_device != "-1":
            ...
            self.translation_stage_controller = controller.translation_stage_controller(config=self.configs,pw=self.translation_stage_controller_window,debugprint=self.debugprint)
        ...
        self.diagnostic_particles = diagnostic_particles.diagnostic_particles(config=self.configs,pw=self.diagnostic_particles_window,debugprint=self.debugprint,controller=self.controller)
        ...
        if self.translation_stage_device != "-1":
            ...
            self.translation_stage = translation_stage.translation_stage(config=self.configs,pw=self.translation_stage_window,debugprint=self.debugprint,controller=self.controller)
        ...
        self.info_area_setpoints['previous setpoint'] = tkinter.Button(pw,text="previous",command=self.choose_previous_setpoint,padx=self.padx,pady=self.pady,state=tkinter.DISABLED)
    ...
    def create_debug_area(self):
        if (int(self.args.debug)==1):
            ...
            self.debug = 1
            self.debug_infos_message = tkinter.Label(self.debug_infos,textvariable=self.debugtext,height=self.configs.values.getint('gui','debug_area_height'),width=self.configs.values.getint('gui','debug_area_width'),anchor="sw",justify=tkinter.LEFT)
            #self.debugtext.set("")
    ...
    def exit(self):
        ...
        if self.configs.values.getboolean('environment_sensor_5','stop_sensor_on_exit'):
            c = ['killall',self.configs.values.get('environment_sensor_5','command')]
            try:
                prc_srv = subprocess.Popen(c)
            except: pass
        try:
    ...
    def start_extern_program_debug_controller(self):
        ...
        if (self.configname != None) and (len(self.configname)>0):
            cc = [c,
                  "-system_config",self.args.system_config,
    ...
    def load_setpoints(self):
        if self.setpoints_choose != None:
            i = self.setpoints_choose
            s = self.setpoints.sections()[i]
            log.debug("load setpoints %d: %s" % (i,s))
            if (self.setpoints.has_option(s,'load_set') and
                self.setpoints.getboolean(s,'load_set')):
                self.set_setpoints()
