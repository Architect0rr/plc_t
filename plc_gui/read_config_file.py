...
class read_config_file():
    ...
    Date: 2013-05-14
    """
    def __init__(self,system_wide_ini_file="/etc/plc.cfg",user_ini_file="~/.plc.cfg"):
        ...
        for i in range(self.number_of_cameras):
            if (self.values.has_section("camera%d" % (i+1)) and
                self.values.has_option("camera%d" % (i+1),"guid") and
                (self.values.get("camera%d" % (i+1),'guid') != '-2')):
                n += 1
            else:
                break
        ...
        for i in range(self.number_of_acceleration_sensor):
            if (self.values.has_section("acceleration_sensor%d" % (i+1)) and
                self.values.has_option("acceleration_sensor%d" % (i+1),"SerialNumber") and
                (self.values.get("acceleration_sensor%d" % (i+1),'SerialNumber') != '-2')):
                n += 1
            else:
                break
        self.number_of_acceleration_sensor = n

    def set_default(self,c,system_wide_ini_file="/etc/plc.cfg",user_ini_file="~/.plc.cfg"):
        # ini
        c.add_section('ini')
    ...
    def write_default_config_file(self,file=None):
        if file != None:
            c = configparser.SafeConfigParser()
            self.set_default(c)
            with open(os.path.expanduser(file), 'wb') as configfile:
                c.write(configfile)
