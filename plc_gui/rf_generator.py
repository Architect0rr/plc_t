...
class rf_generator_gui():
    ...
    def __init__(self,config=None,pw=None,debugprint=None,controller=None):
        ...
        for g in range(3):
            if self.generator[g].exists:
                ...
                self.generator[g].power_status_checkbutton = tkinter.Checkbutton(self.power_frame,text="Power RF-Generator %d" % (g+1),command=self.generator[g].power_cmd,variable=self.generator[g].power_status,state=tkinter.DISABLED)
        ...
        self.pattern_length_val.set(int(round(self.config.values.getfloat('RF-Generator','pattern_microcontroller_intervall_length_factor')*self.config.values.getint('RF-Generator','pattern_microcontroller_min_intervall_length')*1000000)))
        ...
        for g in range(3):
            if self.generator[g].exists:
                for i in range(4):
                    ...
                    self.generator[g].channel[i].onoff_status_checkbutton = tkinter.Checkbutton(self.control_frame,text="Pwr Channel %d" % (cn),command=self.rf_channel_onoff_cmd,variable=self.generator[g].channel[i].onoff_status,state=tkinter.DISABLED)
                    ...
                    self.generator[g].channel[i].choose_checkbutton = tkinter.Checkbutton(self.control_frame,text="Channel %d" % (cn),variable=self.generator[g].channel[i].choose)
    ...
    def pattern_load_file(self):
        if self.pattern_file != None:
            ...
            if (self.pattern_config.has_option('pattern','number_of_generators') and
                self.pattern_config.has_option('pattern','pattern_length') and
                self.pattern_config.has_option('pattern','pattern_intervall_length') and
                self.pattern_config.has_option('pattern','pattern')):
    ...
    def pattern_write_to_generator(self):
        ...
        if self.pattern['controller'] == "microcontroller":
            ...
            self.pattern['pattern_intervall_length'] = max(self.config.values.getint('RF-Generator','pattern_microcontroller_min_intervall_length'),min(self.pattern['pattern_intervall_length'],self.config.values.getint('RF-Generator','pattern_microcontroller_max_intervall_length')))
            self.pattern_length_val.set(self.pattern['pattern_intervall_length'])
        ...
        if len(self.pattern['pattern']) == self.pattern['number_of_generators']*4*self.pattern['pattern_length']:
            if self.pattern['controller'] == "computer":
                log.debug("initiate using pattern '%s' with length %d in %d generators" % (self.pattern['pattern'],self.pattern['pattern_length'],self.pattern['number_of_generators']))
            else:
                log.debug("initiate writing pattern '%s' with length %d to %d generators" % (self.pattern['pattern'],self.pattern['pattern_length'],self.pattern['number_of_generators']))
