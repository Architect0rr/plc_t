...
class gui:
    def __init__(
        ...
        timestamp_min=0,
        timestamp_max=0,
        minupdateinterval1=6,
        maxupdateinterval1=42,
        minupdateinterval2=1,
        maxupdateinterval2=23,
        create_info_graphics=None,
        gamma=1.0,
    ):
        self.log = log
        ...
        self.minus100sbutton = tkinter.Button(
            self.controlframe, text="-100s", command=self.gominus100s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.minus100sbutton.pack(side=tkinter.LEFT)
        self.minus10sbutton = tkinter.Button(
            self.controlframe, text="-1s", command=self.gominus10s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.minus10sbutton.pack(side=tkinter.LEFT)
        self.minus1sbutton = tkinter.Button(
            self.controlframe, text="-0.01s", command=self.gominus1s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.minus1sbutton.pack(side=tkinter.LEFT)
        self.minus1button = tkinter.Button(
            self.controlframe, text="-1", command=self.gominus1, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        ...
        self.plus1button = tkinter.Button(
            self.controlframe, text="+1", command=self.goplus1, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.plus1button.pack(side=tkinter.LEFT)
        self.plus1sbutton = tkinter.Button(
            self.controlframe, text="+0.01s", command=self.goplus1s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.plus1sbutton.pack(side=tkinter.LEFT)
        self.plus10sbutton = tkinter.Button(
            self.controlframe, text="+1s", command=self.goplus10s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        self.plus10sbutton.pack(side=tkinter.LEFT)
        self.plus100sbutton = tkinter.Button(
            self.controlframe, text="+100s", command=self.goplus100s, padx=self.padx, pady=self.pady, repeatdelay=self.repeatdelay, repeatinterval=self.repeatinterval
        )
        ...
        for i in range(len(self.timestamps)):
            ...
            if gx > gxm:
                gx = 0
                gy = gy + 1
            self.outframes[i]["playgui"] = play_gui(
                log=self.log,
                parent=self.outframes[i]["frame"],
                db=self.timestamps[i],
                timeratefactor=self.timeratefactor,
                scale=self.scale,
                absolutscale=self.absolutscale,
                mt=self.mt,
                minupdateinterval1=self.minupdateinterval1,
                maxupdateinterval1=self.maxupdateinterval1,
                minupdateinterval2=self.minupdateinterval2,
                maxupdateinterval2=self.maxupdateinterval2,
                gamma=self.gamma,
            )
        ...
        if self.mpclogdata != None:
            ...
            if gx > gxm:
                gx = 0
                gy = gy + 1
            self.outframes[i]["playgui"] = play_log_gui(
                log=self.log,
                parent=self.outframes[i]["frame"],
                mpclogdata=self.mpclogdata,
        ...
        if self.rflogdata != None:
            ...
            if gx > gxm:
                gx = 0
                gy = gy + 1
            self.outframes[i]["playgui"] = play_rflog_gui(
                log=self.log, parent=self.outframes[i]["frame"], rflogdata=self.rflogdata, configs=self.configs, timeratefactor=self.timeratefactor, mt=self.mt
            )
    ...
    def create_infoframe_pictures(self, createtime=False):
        if createtime:
            # create only time
            for i in range(len(self.timestamps)):
                self.brightnesspic[i] = self.create_image_from_array(
                    self.graphic1_width, self.graphic1_height, self.brightness, i, createtime=createtime, m=self.master_brightnesspic[i].copy()
                )
            ...
            if self.infoframestatus == 6:
                self.infoframe_all_pic = self.create_image_from_arrays(
                    self.graphic1_width, self.graphic1_height, self.brightness, self.diffs, createtime=createtime, m=self.master_infoframe_all_pic.copy()
                )
