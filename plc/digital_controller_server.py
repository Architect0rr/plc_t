...
class controller_class(plc_tools.server_base.controller_class):
    ...
    Date: 2012-09-30
    """
    def myinit(self,A=None,B=None,C=None,D=None,simulate=False):
        ...
        self.actualvalue['dispenser'] = {'n': None, 'ton': None, 'shake': False, 'port': None, 'channel': None, 'toff': None}
        self.setpointlock.acquire() # lock for running
        self.setpoint = self.actualvalue.copy()
        self.setpointlock.release() # release the lock
        self.actualvalue = {'A': 8*[None], 'B': 8*[None], 'dispenser': {'n': None, 'ton': None, 'shake': False, 'port': None, 'channel': None, 'toff': None}, 'C': 8*[None], 'D': 8*[None]}
    ...
    def shake_dispenser(self):
        ...
        if (self.setpoint['dispenser']['shake'] and
            (self.actualvalue['dispenser']['n'] != None) and
            (self.actualvalue['dispenser']['ton'] != None) and
            (self.actualvalue['dispenser']['port'] != None) and
            (self.actualvalue['dispenser']['channel'] != None) and
            (self.actualvalue['dispenser']['toff'] != None)):
            self.log.debug("shake with %d %f %f on %s %d" % (self.actualvalue['dispenser']['n'],self.actualvalue['dispenser']['ton'],self.actualvalue['dispenser']['toff'],self.actualvalue['dispenser']['port'],self.actualvalue['dispenser']['channel']))
            self.setpoint['dispenser']['shake'] = False
            self.actualvalue['dispenser']['shake'] = True
            t0 = time.time()
            for i in range(self.actualvalue['dispenser']['n']):
                t1 = time.time()
                self.actualvalue[self.actualvalue['dispenser']['port']][self.actualvalue['dispenser']['channel']] = True # ON
                s = ['@%s%02X' % (self.actualvalue['dispenser']['port'],self.boolarray2int(self.actualvalue[self.actualvalue['dispenser']['port']]))]
                self.write_to_device(s)
                t = self.actualvalue['dispenser']['ton']-(time.time()-t1)
                if t > 0:
                    time.sleep(t)
                t2 = time.time()
                self.actualvalue[self.actualvalue['dispenser']['port']][self.actualvalue['dispenser']['channel']] = False # OFF
                s = ['@%s%02X' % (self.actualvalue['dispenser']['port'],self.boolarray2int(self.actualvalue[self.actualvalue['dispenser']['port']]))]
                self.write_to_device(s)
    ...
    def write_to_device(self,s):
        ...
        if self.device == None or self.devicename == "":
            ...
            #self.log.debug("out: %s" % string.join(s,""))
    ...
    def set_setpoint_human_readable(self,port=None,channel=None,v=False):
    ...
    def set_setpoint_int(self,v=None):
        self.setpointlock.acquire() # lock to set
        self.setpoint['A'] = self.int2boolarray(struct.unpack("B",v[0])[0])
        self.setpoint['B'] = self.int2boolarray(struct.unpack("B",v[1])[0])
        self.setpoint['C'] = self.int2boolarray(struct.unpack("B",v[2])[0])
        self.setpoint['D'] = self.int2boolarray(struct.unpack("B",v[3])[0])
        self.setpointlock.release() # release the lock
        ...
        while controller.running:
            ...
            while found_something:
                ...
                if (len(data) >= 2) and (data[0].lower() == "p"):
                    ...
                    controller.set_setpoint_human_readable('dispenser','port',data[10])
                    controller.set_setpoint_human_readable('dispenser','channel',int(data[11]))
                    controller.set_setpoint_human_readable('dispenser','n',int(data[12:14]))
                    controller.set_setpoint_human_readable('dispenser','ton',int(data[14:17])/1000.0)
                    controller.set_setpoint_human_readable('dispenser','toff',int(data[17:20])/1000.0)
                    data = data[20:]
                elif (len(data) >= 10) and (data[0:10].lower() == "!dispenser"):
                    # !dispenser
                    # do the shake
                    found_something = True
                    if (controller.actualvalue['dispenser']['n'] != None and
                        controller.actualvalue['dispenser']['ton'] != None and
                        controller.actualvalue['dispenser']['port'] != None and
                        controller.actualvalue['dispenser']['channel'] != None and
                        controller.actualvalue['dispenser']['toff'] != None):
                    ...
                    found_something = True
                    a = "digital_controller_server Version: %s" % __digital_controller_server_version__
                    response += a
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Author: Daniel Mohr\nDate: %s\nLicense: GNU GENERAL PUBLIC LICENSE, Version 3, 29 June 2007\n\n%s" % (__digital_controller_server_date__,help),
    ...
    parser.add_argument('-logfile',
                        ...
                        required=False,
                        dest='logfile',
                        help='Set the logfile to f. The WatchedFileHandler is used. This means, the logfile grows indefinitely until an other process (e. g. logrotate or the user itself) move or delete the logfile. Under Windows moving or deleting of open files is impossible and therefore the logfile grows indefinitely. default: %s' % os.path.join(tempfile.gettempdir(),"digital_controller.log"),
                        metavar='f')
    parser.add_argument('-datalogfile',
                        ...
                        required=False,
                        dest='datalogfile',
                        help='Set the datalogfile to f. Only the measurements will be logged here. The WatchedFileHandler is used. This means, the logfile grows indefinitely until an other process (e. g. logrotate or the user itself) move or delete the logfile. Under Windows moving or deleting of open files is impossible and therefore the logfile grows indefinitely. default: %s' % os.path.join(tempfile.gettempdir(),"digital_controller.data"),
                        metavar='f')
    parser.add_argument('-runfile',
                        ...
                        required=False,
                        dest='runfile',
                        help='Set the runfile to f. If an other process is running with a given pid and writing to the same device, the program will not start. Setting f=\"\" will disable this function. default: %s' % os.path.join(tempfile.gettempdir(),"digital_controller.pids"),
                        metavar='f')
    ...
    log.info("start logging in digital_controller_server: %s" % time.strftime("%a, %d %b %Y %H:%M:%S %z %Z", time.localtime()))
    ...
    controller = controller_class(log,datalog,args.device,args.timedelay,args.A,args.B,args.C,args.D,args.simulate)
    s = True
    while s:
        s = False
        try:
