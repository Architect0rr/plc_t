...
class electrode_motion_controller:
    ...
    def __init__(self, config: read_config_file.read_config_file, pw: tkinter.LabelFrame, debugprint: Callable[[str], None]) -> None:
        ...
        self.pw = pw
        self.debugprint: Callable[[str], None] = debugprint
        ...
        self.update_intervall = int(self.config.values.get("electrode motion controller", "update_intervall"))
        ...
        self.setpoint: Dict[str, int] = dict()
    ...
    def update(self, restart: bool = True):
        ...
        selfrestart = False
        write2dev = False
        write2dev_string: str = ""
        if self.actualvalue["connect"]:
            selfrestart = True
            if self.setpoint["upper guard ring"] is not None and self.setpoint["upper guard ring"] != 0:
                # move upper guard
                ...
                write2dev = True
            elif self.setpoint["upper guard ring"] is not None and self.setpoint["upper guard ring"] == 0:
                self.setpoint["upper guard ring"] = 0
            if self.disabled_lower_guard_ring:
                self.setpoint["lower guard ring"] = 0
            elif self.setpoint["lower guard ring"] is not None and self.setpoint["lower guard ring"] != 0:
                # move lower guard
                ...
                self.setpoint["lower guard ring"] = 0
            if self.setpoint["upper electrode"] is not None and self.setpoint["upper electrode"] != 0:
                ...
                write2dev_string = "%s%s" % (write2dev_string, cmd)
                write2dev = True
            elif self.setpoint["upper electrode"] is not None and self.setpoint["upper electrode"] == 0:
                self.setpoint["upper electrode"] = 0
            if self.setpoint["lower electrode"] is not None and self.setpoint["lower electrode"] != 0:
                ...
                write2dev_string = "%s%s" % (write2dev_string, cmd)
                write2dev = True
            elif self.setpoint["lower electrode"] is not None and self.setpoint["lower electrode"] == 0:
                self.setpoint["lower electrode"] = 0
            l: str = ""
            try:
