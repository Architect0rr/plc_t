...
class gas_system:
    ...
    def __init__(self, config: read_config_file, pw: tkinter.ttk.LabelFrame, _log: logging.Logger, controller: Dict):
    ...
    def membran_pump(self):
        if self.membran_pump_status.get() == 0:
            ...
            s = True
        self.log.debug("switch membran pump to %s" % b2onoff(s))
        ctr = self.config.values.get("gas system", "membran_pump_status_controller")
        self.controller[ctr].lock.acquire()  # lock
        self.controller[ctr].setpoint[self.config.values.get("gas system", "membran_pump_status_port")][
            int(self.config.values.get("gas system", "membran_pump_status_channel"))
        ] = s
    ...
    def turbo_pump_1(self):
        if self.turbo_pump_1_status.get() == 0:
            ...
            s = True
        self.log.debug("switch turbo pump 1 to %s" % b2onoff(s))
        ctr = self.config.values.get("gas system", "turbo_pump_1_status_controller")
        self.controller[ctr].lock.acquire()  # lock
        self.controller[ctr].setpoint[self.config.values.get("gas system", "turbo_pump_1_status_port")][
            int(self.config.values.get("gas system", "turbo_pump_1_status_channel"))
        ] = s
        self.controller[ctr].lock.release()  # release the lock
    ...
    def turbo_pump_2(self):
        if self.turbo_pump_2_status.get() == 0:
            ...
            s = True
        self.log.debug("switch turbo pump 2 to %s" % b2onoff(s))
        ctr = self.config.values.get("gas system", "turbo_pump_2_status_controller")
        self.controller[ctr].lock.acquire()  # lock
        self.controller[ctr].setpoint[self.config.values.get("gas system", "turbo_pump_2_status_port")][
            int(self.config.values.get("gas system", "turbo_pump_2_status_channel"))
        ] = s
        self.controller[ctr].lock.release()  # release the lock
    ...
    def mass_flow(self):
        if self.mass_flow_status.get() == 0:
            ...
            s = True
        self.log.debug("switch mass flow controller to %s" % b2onoff(s))
    ...
    def mass_flow_set_flow_rate_command(self):
        ...
        if len(v) > 0:
            ...
            self.log.debug("set flow rate to (%f V, %f msccm)" % (vo, v))
            self.controller[ctr].lock.acquire()  # lock
            try:
    ...
    def update(self):
        ...
        self.membran_pump_status_val.set(
            b2onoff(
                self.controller[self.config.values.get("gas system", "membran_pump_status_controller")].actualvalue[
                    self.config.values.get("gas system", "membran_pump_status_port")
                ][int(self.config.values.get("gas system", "membran_pump_status_channel"))]
            )
        )

        self.turbo_pump_1_status_val.set(
            b2onoff(
                self.controller[self.config.values.get("gas system", "turbo_pump_1_status_controller")].actualvalue[
                    self.config.values.get("gas system", "turbo_pump_1_status_port")
                ][int(self.config.values.get("gas system", "turbo_pump_1_status_channel"))]
            )
        )
        self.turbo_pump_2_status_val.set(
            b2onoff(
                self.controller[self.config.values.get("gas system", "turbo_pump_2_status_controller")].actualvalue[
                    self.config.values.get("gas system", "turbo_pump_2_status_port")
                ][int(self.config.values.get("gas system", "turbo_pump_2_status_channel"))]
            )
        )
        # RPM
        ...
        self.turbo_pump_1_rpm_val.set(
            "%d %%"
            % round(
                (
                    self.controller[self.config.values.get("gas system", "turbo_pump_1_rpm_controller")].actualvalue[
                        self.config.values.get("gas system", "turbo_pump_1_rpm_port")
                    ][int(self.config.values.get("gas system", "turbo_pump_1_rpm_channel"))]
                    + 0.0
                )
                * 100
                / 10.0
        ...
        self.turbo_pump_2_rpm_val.set(
            "%d %%"
            % round(
                (
                    self.controller[self.config.values.get("gas system", "turbo_pump_2_rpm_controller")].actualvalue[
                        self.config.values.get("gas system", "turbo_pump_2_rpm_port")
                    ][int(self.config.values.get("gas system", "turbo_pump_2_rpm_channel"))]
                    + 0.0
                )
                * 100
                / 10.0
        ...
        err2 = False
        if not (
            self.controller[self.config.values.get("gas system", "turbo_pump_1_error_rotation_controller")].actualvalue[
                self.config.values.get("gas system", "turbo_pump_1_error_rotation_port")
            ][int(self.config.values.get("gas system", "turbo_pump_1_error_rotation_channel"))]
        ):
            err1 = True
        if not (
            self.controller[self.config.values.get("gas system", "turbo_pump_1_error_general_controller")].actualvalue[
                self.config.values.get("gas system", "turbo_pump_1_error_general_port")
            ][int(self.config.values.get("gas system", "turbo_pump_1_error_general_channel"))]
        ):
            err2 = True
        err = ""
        if err1:
        ...
        err2 = False
        if not (
            self.controller[self.config.values.get("gas system", "turbo_pump_2_error_rotation_controller")].actualvalue[
                self.config.values.get("gas system", "turbo_pump_2_error_rotation_port")
            ][int(self.config.values.get("gas system", "turbo_pump_2_error_rotation_channel"))]
        ):
            err1 = True
        if not (
            self.controller[self.config.values.get("gas system", "turbo_pump_2_error_general_controller")].actualvalue[
                self.config.values.get("gas system", "turbo_pump_2_error_general_port")
            ][int(self.config.values.get("gas system", "turbo_pump_2_error_general_channel"))]
        ):
            err2 = True
        if err1:
            if err2:
