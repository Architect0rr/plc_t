...
class PLC(ttk.Frame):
    ...
    @classmethod
    def main(cls, args, log, conigname):
        # tkinter.NoDefaultRoot()
        root = tkinter.Tk()
        app = cls(root, args, log, conigname)
    ...
    def __init__(self, root: tkinter.Tk, args: argparse.Namespace, _log: logging.Logger, configname: Optional[str] = None) -> None:
        """
        GUI initialization
        """
        # set variables
        ...
        if (configname is not None) and (len(configname) > 0):
            self.main_window.title(f"plc - PlasmaLabControl ({configname})")
        else:
