...
class electrode_motion:
    ...
    def __init__(self, config: read_config_file, pw: ttk.LabelFrame, _log: logging.Logger, controller: Dict) -> None:
        ...
        self.start_controller_button = tkinter.Button(
            self.init_frame,
            text="open RS232 (emc)",
            command=self.controller["emc"].start_request,
            padx=self.padx,
            pady=self.pady,
        )
        ...
        self.stop_controller_button = tkinter.Button(
            self.init_frame,
            text="close RS232 (emc)",
            command=self.controller["emc"].stop_request,
            padx=self.padx,
            pady=self.pady,
        )
        ...
        self.lower_guard_ring: Dict[str, Any] = {}
        ...
        self.lower_guard_ring["10 down button"] = tkinter.Button(
            f, command=self.lower_guard_ring["10 down cmd"], text="10 down", padx=self.padx, pady=self.pady
        )
        self.lower_guard_ring["10 down button"].grid(column=0, row=0)
        self.lower_guard_ring["1 down button"] = tkinter.Button(
            f, command=self.lower_guard_ring["1 down cmd"], text="1 down", padx=self.padx, pady=self.pady
        )
        ...
        self.lower_guard_ring["n up/down entry"] = tkinter.Entry(
            self.lower_guard_ring["n up/down frame"], textvariable=self.lower_guard_ring["n up/down val"], width=6
        )
        self.lower_guard_ring["n up/down entry"].grid(column=0, row=0)
        self.lower_guard_ring["n up/down button"] = tkinter.Button(
            self.lower_guard_ring["n up/down frame"],
            command=self.lower_guard_ring["n up/down cmd"],
            text="do",
            padx=self.padx,
            pady=self.pady,
        )
        self.lower_guard_ring["n up/down button"].grid(column=1, row=0)
        self.lower_guard_ring["1 up button"] = tkinter.Button(
            f, command=self.lower_guard_ring["1 up cmd"], text="1 up", padx=self.padx, pady=self.pady
        )
        self.lower_guard_ring["1 up button"].grid(column=3, row=0)
        self.lower_guard_ring["10 up button"] = tkinter.Button(
            f, command=self.lower_guard_ring["10 up cmd"], text="10 up", padx=self.padx, pady=self.pady
        )
        self.lower_guard_ring["10 up button"].grid(column=4, row=0)
        # upper guard ring
        f = self.upper_guard_ring_frame
        self.upper_guard_ring: Dict[str, Any] = {}
        ...
        self.upper_guard_ring["10 down button"] = tkinter.Button(
            f, command=self.upper_guard_ring["10 down cmd"], text="10 down", padx=self.padx, pady=self.pady
        )
        self.upper_guard_ring["10 down button"].grid(column=0, row=0)
        self.upper_guard_ring["1 down button"] = tkinter.Button(
            f, command=self.upper_guard_ring["1 down cmd"], text="1 down", padx=self.padx, pady=self.pady
        )
        ...
        self.upper_guard_ring["n up/down entry"] = tkinter.Entry(
            self.upper_guard_ring["n up/down frame"], textvariable=self.upper_guard_ring["n up/down val"], width=6
        )
        self.upper_guard_ring["n up/down entry"].grid(column=0, row=0)
        self.upper_guard_ring["n up/down button"] = tkinter.Button(
            self.upper_guard_ring["n up/down frame"],
            command=self.upper_guard_ring["n up/down cmd"],
            text="do",
            padx=self.padx,
            pady=self.pady,
        )
        self.upper_guard_ring["n up/down button"].grid(column=1, row=0)
        self.upper_guard_ring["1 up button"] = tkinter.Button(
            f, command=self.upper_guard_ring["1 up cmd"], text="1 up", padx=self.padx, pady=self.pady
        )
        self.upper_guard_ring["1 up button"].grid(column=3, row=0)
        self.upper_guard_ring["10 up button"] = tkinter.Button(
            f, command=self.upper_guard_ring["10 up cmd"], text="10 up", padx=self.padx, pady=self.pady
        )
        self.upper_guard_ring["10 up button"].grid(column=4, row=0)

        # lower electrode
        f = self.lower_electrode_frame
        self.lower_electrode: Dict[str, Any] = {}
        ...
        self.lower_electrode["10 down button"] = tkinter.Button(
            f, command=self.lower_electrode["10 down cmd"], text="10 down", padx=self.padx, pady=self.pady
        )
        self.lower_electrode["10 down button"].grid(column=0, row=0)
        self.lower_electrode["1 down button"] = tkinter.Button(
            f, command=self.lower_electrode["1 down cmd"], text="1 down", padx=self.padx, pady=self.pady
        )
        ...
        self.lower_electrode["n up/down entry"] = tkinter.Entry(
            self.lower_electrode["n up/down frame"], textvariable=self.lower_electrode["n up/down val"], width=6
        )
        self.lower_electrode["n up/down entry"].grid(column=0, row=0)
        self.lower_electrode["n up/down button"] = tkinter.Button(
            self.lower_electrode["n up/down frame"],
            command=self.lower_electrode["n up/down cmd"],
            text="do",
            padx=self.padx,
            pady=self.pady,
        )
        self.lower_electrode["n up/down button"].grid(column=1, row=0)
        self.lower_electrode["1 up button"] = tkinter.Button(
            f, command=self.lower_electrode["1 up cmd"], text="1 up", padx=self.padx, pady=self.pady
        )
        self.lower_electrode["1 up button"].grid(column=3, row=0)
        self.lower_electrode["10 up button"] = tkinter.Button(
            f, command=self.lower_electrode["10 up cmd"], text="10 up", padx=self.padx, pady=self.pady
        )
        self.lower_electrode["10 up button"].grid(column=4, row=0)
        # upper electrode
        f = self.upper_electrode_frame
        self.upper_electrode: Dict[str, Any] = {}
        ...
        self.upper_electrode["10 down button"] = tkinter.Button(
            f, command=self.upper_electrode["10 down cmd"], text="10 down", padx=self.padx, pady=self.pady
        )
        self.upper_electrode["10 down button"].grid(column=0, row=0)
        self.upper_electrode["1 down button"] = tkinter.Button(
            f, command=self.upper_electrode["1 down cmd"], text="1 down", padx=self.padx, pady=self.pady
        )
        ...
        self.upper_electrode["n up/down entry"] = tkinter.Entry(
            self.upper_electrode["n up/down frame"], textvariable=self.upper_electrode["n up/down val"], width=6
        )
        self.upper_electrode["n up/down entry"].grid(column=0, row=0)
        self.upper_electrode["n up/down button"] = tkinter.Button(
            self.upper_electrode["n up/down frame"],
            command=self.upper_electrode["n up/down cmd"],
            text="do",
            padx=self.padx,
            pady=self.pady,
        )
        self.upper_electrode["n up/down button"].grid(column=1, row=0)
        self.upper_electrode["1 up button"] = tkinter.Button(
            f, command=self.upper_electrode["1 up cmd"], text="1 up", padx=self.padx, pady=self.pady
        )
        self.upper_electrode["1 up button"].grid(column=3, row=0)
        self.upper_electrode["10 up button"] = tkinter.Button(
            f, command=self.upper_electrode["10 up cmd"], text="10 up", padx=self.padx, pady=self.pady
        )
    ...
    def power(self):
        if self.power_status.get() == 0:
            ...
            s = True
        self.log.debug("switch electrode motion to %s" % b2onoff(s))
        self.controller[self.config.values.get("electrode motion controller", "power_controller")].setpoint[
            self.config.values.get("electrode motion controller", "power_port")
        ][int(self.config.values.get("electrode motion controller", "power_channel"))] = s
    ...
    def check_buttons(self):
        ...
        if self.controller["emc"].actualvalue["connect"]:
            ...
            # lower_guard_ring
            if not self.controller["emc"].disabled_lower_guard_ring:
