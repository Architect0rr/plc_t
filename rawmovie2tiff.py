#!/usr/bin/python -O
# Author: Daniel Mohr
# Date: 2013-09-24, 2014-07-22, 2017-05-30

__rawmovies2tiffs_date__ = "2017-05-30"
__rawmovies2tiffs_version__ = __rawmovies2tiffs_date__
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='rawmovie2tiffs.py',
        epilog="Author: Daniel Mohr\nDate: %s\nLicense: GNU GENERAL PUBLIC LICENSE, Version 3, 29 June 2007\n\nExample: rawmovie2tiffs.py -d 1 -f cam_0_PF2011-CAM3_2012-09-11_006.img -o t" % __rawmovies2tiffs_date__,
    ...
    parser.add_argument('-suffix',
                        nargs=1,
                        default="_[timestamp].tiff",
                        type=str,
                        required=False,
                        dest='suffix',
                        help='Set the suffix for the names. Default: "_[timestamp].tiff". Possible variables in suffix are "[timestamp]" and "[nr]".',
                        metavar='suf')
    ...
    # logging
    log = logging.getLogger('rawmovie2tiffs')
    ...
    log.addHandler(ch)
    log.info("start logging in rawmovie2tiffs: %s" % time.strftime("%a, %d %b %Y %H:%M:%S %z %Z", time.localtime()))
    # do something
    if args.prefixfile != "":
        # read prefixfile
            ...
            if p:
                ...
                if args.info:
                    ...
                    for p in pics:
                        ...
                        #outputname = "%s_%s.tiff" % (outdir,timestamp)
                        output =  open(outputname,'wb')
                        pic.save(output,"TIFF")
                        output.flush()
                        output.close()
                        if (timestamp != None):
                            timestampsoutput = "%s\n%s%03d_%06d.tiff\t%f" % (timestampsoutput,args.prefix,rec,recit,timestamp)
